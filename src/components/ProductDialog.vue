<template>
  ...
  >
    ...
    <b-row v-for="product in products" :key="product.id">
      <b-col class="d-flex align-items-start" cols="4">
        <b-card-img alt="Image" class="rounded-0" :src="product.image"></b-card-img>
      </b-col>
      <b-col cols="8">
        <h6>{{ product.name }}</h6>
        <div v-for="size in product.sizes" :key="size.number">
          <div v-if="size.piece > 0">
            ...
            <button class="btn-decrement" @click="decrement(product, size.number)">-</button>
            <button class="btn-increment" @click="increment(product, size.number)">+</button>
